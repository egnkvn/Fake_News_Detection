...
以下是各工具的檢測結果：
1. 常識合理性檢測：{commonsense_result}
2. 標題-內容一致性：{title_content_result}
3. 網域可信度：{url_result}
4. 煽動性語言檢測：{phrase_result}
5. 語言錯誤檢測：{language_result}

請模擬人類事實查核流程，重點評估「常識合理性」。再來，若標題-內容不一致，則判斷為fake。綜合判斷新聞真偽。請以以下格式回覆。：
Label: <real/fake>
...
if __name__ == "__main__":
    with open('../fake_data/test/real_news2.json') as f:
        data = json.load(f)
    print(len(data), "條測試數據")

    logs = []
    logs_path = "log/logs_no_search_real.jsonl"

    with open(logs_path, "a", encoding="utf-8") as log_f:
        for d in tqdm(data):
            # if d.get("category") not in ["fabricated"]:
            #     continue
            rec = {
                "id":       d.get("id"),
